...
namespace p6
{
	...
	private:
		...
		Coord _old_shift;								///<Old shift when being shifted
		real _old_scale;								///<Old scale rate when being scaled
		real _old_angle;								///<Old angle when being rotated
		Coord _initial_anchor;							///<Anchor's initial (without shift) position
		Coord _drag_begin;								///<Begin point of dragging (in reals, unlike MainPanel)
	...
	public:
		...
		void set_anchor(wxPoint point)		noexcept;		///<Sets coordinates of anchor
		void drag_begin(wxPoint point)		noexcept;		///<Begins dragging by mouse
		void drag_continue(wxPoint point)	noexcept;		///<Processes dragging by mouse
		void show()							noexcept;		///<Adds bar's components to side bar
		void refresh()						noexcept;		///<Refreshes initial coordinates and directions
