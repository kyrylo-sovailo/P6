...
p6::MenuBar::MenuBar(Frame *frame)
{
	...
	frame->frame->Bind(wxEVT_MENU, &MenuBar::_on_menu_export_png, this, item->GetId());
	item = exportmenu->Append(wxID_ANY, "JPEG", "Export image as JPEG");
	frame->frame->Bind(wxEVT_MENU, &MenuBar::_on_menu_export_jpeg, this, item->GetId());
	menubar->Append(exportmenu, "Export");

	//Help
	wxMenu *helpmenu = new wxMenu();
	item = helpmenu->Append(wxID_ANY, "Help", "Get help");
	frame->frame->Bind(wxEVT_MENU, &MenuBar::_on_menu_help, this, item->GetId());
